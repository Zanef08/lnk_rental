...
<html lang="en">
        ...
        <script>
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                rows.forEach(function (row) {
                    ...
                    } else if (statusCell.textContent === "Waiting") {
    ...
    <body>
        ...
        <nav>
            <ul>
                ...
                <li class="tablinks" onclick="openUser(event, 'MyPost')">POST</li>
                <li class="tablinks" onclick="openUser(event, 'Notifications')">STATUS</li>
            </ul>
        </nav>
    </nav>
    ...
    <div id="MyPost" class="box" style="display: none;">
        <div>
            <form action="InformationController" method="Post">       
                <input type="submit" name="action" value="newPost" style=" background-color: #32B4B7; padding: 12px 28px; border-radius: 5px; border: none; margin-left: 10px; color: white; margin-bottom: 10px;">
            </form>
        </div>
        <div class="box1">
            <table>
                <%
                    List<infoDTO> infoList = (List<infoDTO>) request.getAttribute("infoList");
                    if (!infoList.isEmpty()) {
                        for (infoDTO info : infoList) {
                %>
                <tr>
                    <td>
                        <img src="<%= info.getInforImage()%>" alt="Info Image" class="box-image">
                    </td>
                    <td>
                        <div class="box-info">
                            <h4><%= info.getInfoTitle()%></h4>
                            <button style="background-color: #32B4B7; border: none; padding: 5px; border-radius: 5px;">
                                <%= info.getInfoType()%>
                            </button> 
                            <p style="color: blue; margin: 0;"><%= info.getInfoAddress()%></p>
                        </div>
                    </td>
                    <td>
                        <div class="box-price">
                            <p style="color: red;"><%= info.getInfoPrice()%> VND</p>
                        </div>
                    </td>
                </tr>
                <%
        ...
        <table>
            <thead>
                <tr style="background-color: #0096C7;">
                    <th style="padding: 30px; color: white;" class="sortable">Title</th>
                    <th style="color: white;" class="sortable">Date</th>
                    <th style="color: white;" class="sortable">Status</th>
                </tr>
            </thead>
            <tbody>
                <%
                    List<infoDTO> infoListWait = (List<infoDTO>) request.getAttribute("infoListWait");
                    if (infoList != null && !infoList.isEmpty()) {
                        for (infoDTO info : infoListWait) {
                %>
                <tr class="box2">
                    <td> <%= info.getInfoTitle()%></td>
                    <td> <%= info.getUpDate()%></td>
                    <td><%= info.getInfoStatus()%></td>
                </tr>
                <%
                    }
                } else {
