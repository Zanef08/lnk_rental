...
public class lendlordDAO {
    ...
    public lendlordDTO showLend(String id) {
    ...
    public List<infoDTO> listInfoLend(String lendID) {
        List<infoDTO> infoLendList = new ArrayList<>();
        String sql = "SELECT i.infoTitle, i.inforImage, i.infoType,i.infoAddress,i.infoPrice\n"
                + "FROM information i\n"
                + "join Lendlord l on i.lendID = l.lendID\n"
                + "where l.lendID = ? and i.infoStatus='Approved' ";
        try {
            Connection con = DBUtils.getConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, lendID);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                infoLendList.add(new infoDTO(
                        rs.getString("infoTitle"),
                        rs.getString("inforImage"),
                        rs.getString("infoType"),
                        rs.getString("infoAddress"),
                       
                        rs.getFloat("infoPrice")));
    ...
    public List<infoDTO> listInfoLendWait(String lendID) {
        List<infoDTO> infoLendList = new ArrayList<>();
        String sql = "SELECT i.infoTitle, i.inforImage, i.infoType,i.infoAddress,i.infoPrice, i.UploadDate, i.infoStatus\n"
                + "FROM information i\n"
                + "join Lendlord l on i.lendID = l.lendID\n"
                + "where l.lendID = ?  ";
        try {
