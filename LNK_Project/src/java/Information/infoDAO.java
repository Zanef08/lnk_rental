...
public class infoDAO {
    ...
    public List<infoDTO> getListPostisActive() {
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new infoDTO(rs.getString("infoTitle"), rs.getString("infoAddress"), rs.getString("infoType"), rs.getFloat("infoPrice"), rs.getFloat("couponDiscount")));
            }
            return list;
    ...
    public boolean insertInformation(infoDTO info) {
        ...
        try {
            ...
            // Tạo câu lệnh SQL
            String query = "INSERT INTO [Information] ([lendID], [infoTitle], [infoAddress], [infoType], [infoTypeRoom], [infoPrice], [inforImage], [infoDecription], [uploadDate], [infoStatus])\n"
                    + "VALUES\n"
                    + "(?,  ?, ?, ?, ?, ?, ?, ?, ?, 'Waiting');";
            ...
            // Đặt giá trị các tham số trong câu lệnh SQL
            stmt.setString(1, info.getLendID());
            stmt.setString(2, info.getInfoTitle());
            stmt.setString(3, info.getInfoAddress());
            stmt.setString(4, info.getInfoType());
            stmt.setString(5, info.getInfoTypeRoom());
            stmt.setString(7, info.getInforImage());
            stmt.setString(8, info.getInfoDescription());
            stmt.setDouble(6, info.getInfoPrice());
            stmt.setDate(9, java.sql.Date.valueOf(today));
