...
public class contractDAO {
    ...
    public List<contractDTO> getContractByID(int ID) throws ClassNotFoundException {
        List<contractDTO> list = new ArrayList<>();
        String sql = "SELECT [contractID],[infoTitle],[BookingDate],[Checkin],[Checkout],[contractStatus] FROM [dbo].[Contract] C JOIN [dbo].[Information] I ON C.[infoID] = I.[infoID] WHERE [contractID] = ?";
        try {
            Connection con = DBUtils.getConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, ID);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new contractDTO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6)));
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
