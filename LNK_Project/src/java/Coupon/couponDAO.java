...
public class couponDAO {
    ...
    public List<couponDTO> getCouponByID(int ID) throws ClassNotFoundException {
        ...
        String sql = "SELECT [couponID],[couponName],[couponDetails],[couponFor], [startDate], [expiredDate], [couponStatus] FROM [dbo].[Coupon] WHERE [couponID] = ? ";
        try {
            Connection con = DBUtils.getConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, ID);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list2.add(new couponDTO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7)));
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
